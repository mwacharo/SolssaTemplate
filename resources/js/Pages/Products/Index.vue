                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
                    <div class="lg:col-span-1">
                        <input
                            v-model="searchQuery"
                            type="text"
                            placeholder="Search for a product (id, name, sku)"
                    ...
                    <table class="w-full">
                            ...
                            >
                                <td class="px-4 py-4">
                                    <div class="flex flex-col">
                                        <span
                                            class="bg-red-500 text-white px-2 py-1 rounded text-xs font-medium mb-1 inline-block w-fit"
                                        >
                                            pdt-{{ product.id }}
                                        </span>
                                        <span class="text-sm text-gray-600">{{
                                ...
                                <td class="px-4 py-4 text-center">
                                    <span
                                        class="px-2 py-1 bg-purple-100 text-purple-800 rounded text-sm font-medium"
                                    >
                                        Total {{ getStockDelivered(product) }}
                                    </span>
                                    <div
                                        class="text-xs text-gray-500 mt-1 flex items-center justify-center gap-1"
                                    >
                      ...
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                        ...
                                        <button
                                            @click="openMoveStockModal(product)"
                    ...
                    <div class="mt-3">
                        ...
                        <form @submit.prevent="saveProduct" class="space-y-4">
                            ...
                            <div class="mt-4">
                                ...
                                >
                                <div
                                    v-for="(offer, idx) in form.price_offers"
                                    :key="idx"
                                    ...
                                    >
                                        ...
                                        <div>
                                            <input
                                                v-model="offer.label"
                                                type="text"
                                                placeholder="Offer Label (e.g. Discount, Promo)"
                                                class="w-full px-2 py-1 border rounded"
                                            />
                                        </div>
                                    </div>
                                ...
                                <div
                                    v-if="
                                        form.price_offers &&
                                        form.price_offers.length
                                    "
                                    class="mt-4"
                                >
                                    <table
                                        class="min-w-full divide-y divide-gray-200 border rounded"
                                    >
                                        <thead class="bg-gray-100">
                                            <tr>
                                                <th
                                                    class="px-4 py-2 text-left text-xs font-medium text-gray-700"
                                                >
                                                    Amount
                                                </th>
                                                <th
                        ...
                        >
                            <div>
                                ...
                                <p class="text-sm text-gray-900">
                                    {{ stockForm.product?.product_name }}
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                >
                                <p class="text-sm text-gray-900">
                                    {{ getCurrentStock(stockForm.product) }}
                                </p>
                            </div>
...
const openStockDialog = (product) => {
    selectedProduct.value = {
        ...
        delivered_stock: getStockDelivered(product),
...
const getProductImage = (product) => {
    const primaryImage =
        product.media?.find((m) => m.media_type === "image" && m.is_primary) ||
...
const getStockDelivered = (product) => {
    return product.stocks?.[0]?.stock_delivered || 0;
};
...
const getProductPrice = (product) => {
    ...
    if (product.prices && product.prices.length > 0) {
        ...
        return Number(
            (activePrice?.base_price ?? product.prices[0].base_price) || 0,
        );
    }
...
const saveProduct = async () => {
    try {
        ...
        if (form.value.price_offers && form.value.price_offers.length) {
            formData.append(
                "price_offers",
                JSON.stringify(form.value.price_offers),
            );
        }
