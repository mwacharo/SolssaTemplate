    ...
    <AppLayout>
        ...
        <v-dialog v-model="rolesDialog" max-width="600px" persistent>
            <v-card>
                <v-card-title class="d-flex align-center bg-primary text-white">
                    <span class="text-h6"
                        >Manage Roles - {{ selectedUser?.name }}</span
                    >
                    <v-spacer></v-spacer>
                    <v-btn
                        icon="mdi-close"
        ...
        <v-dialog v-model="permissionsDialog" max-width="600px" persistent>
            <v-card>
                <v-card-title class="d-flex align-center bg-primary text-white">
                    <span class="text-h6"
                        >Manage Permissions - {{ selectedUser?.name }}</span
                    >
                    <v-spacer></v-spacer>
                    <v-btn
                        icon="mdi-close"
                ...
                <v-card-text class="pt-4">
                    <v-alert type="info" variant="tonal" class="mb-4">
                        <div class="text-body-2">
                            ...
                            {{ roleBasedPermissions.join(", ") || "None" }}
                        </div>
                    </v-alert>
                        ...
                        >
                            <template #prepend>
                                <v-checkbox
                                    ...
                                    "
                                    @update:model-value="
                                        togglePermission(permission.key, $event)
                                    "
                                    color="primary"
                                    :loading="
                ...
                <v-card-actions class="pa-4">
                    <div class="text-caption text-medium-emphasis">
                        {{ selectedUserPermissions.length }} direct permissions
                        assigned
                    </div>
                    <v-spacer></v-spacer>
                    <v-btn
        ...
        <v-dialog v-model="servicesDialog" max-width="700px" persistent>
            <v-card>
                <v-card-title class="d-flex align-center bg-primary text-white">
                    <span class="text-h6"
                        >Manage Services - {{ selectedUser?.name }}</span
                    >
                    <v-spacer></v-spacer>
                    <v-btn
                        icon="mdi-close"
                ...
                <v-card-text class="pt-4">
                    <v-alert type="warning" variant="tonal" class="mb-4">
                        Services assigned to sellers determine their billing
                        structure and available features.
                    </v-alert>
                    ...
                    >
                        <v-card-title class="text-subtitle-1 bg-grey-lighten-4"
                            >Assigned Services</v-card-title
                        >
                        <v-list>
                            <v-list-item
                    ...
                    <v-expand-transition>
                        ...
                        >
                            <v-card-title
                                class="text-subtitle-1 bg-grey-lighten-4"
                                >Assign Service</v-card-title
                            >
                            <v-card-text class="pt-4">
                                <v-form
                                    ref="serviceForm"
...
const headers = [
    ...
    { title: "Client Name", value: "client_name", sortable: true },
...
const rules = {
    ...
    minLength: (value) =>
        (value && value.length >= 2) || "Minimum 2 characters required",
    minPassword: (value) =>
...
async function assignRole(userId, roleName) {
    ...
    try {
        await axios.post(`/api/v1/admin/users/${userId}/assign-role`, {
            role: roleName,
        });
...
async function removeRole(userId, roleName) {
    ...
    try {
        await axios.post(`/api/v1/admin/users/${userId}/remove-role`, {
            role: roleName,
        });
...
async function assignPermission(userId, permission) {
    ...
    try {
        await axios.post(`/api/v1/admin/users/${userId}/assign-permission`, {
            permission,
        });
...
async function removePermission(userId, permission) {
    ...
    try {
        await axios.post(`/api/v1/admin/users/${userId}/remove-permission`, {
            permission,
        });
        ...
        const userIndex = users.value.findIndex((u) => u.id === userId);
        if (userIndex !== -1) {
            users.value[userIndex].permissions = users.value[
                userIndex
...
function closeUserDialog() {
    ...
    if (
        userForm.value &&
        typeof userForm.value.resetValidation === "function"
    ) {
...
function cancelServiceForm() {
    ...
    if (
        serviceForm.value &&
        typeof serviceForm.value.resetValidation === "function"
    ) {
