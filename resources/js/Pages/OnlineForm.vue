...
const _populateFromOrder = computed(() => {
  ...
  if (!formData.value.address) {
    const addr =
      o.shipping_address ||
      o.pickup_address ||
      (Array.isArray(o.addresses) && o.addresses.length ? o.addresses[0] : null);
    // if address is an object, attempt common fields, otherwise use string
    if (addr && typeof addr === 'object') {
      formData.value.address =
        addr.full_address || addr.address || `${addr.street || ''} ${addr.area || ''}`.trim() || '';
    } else {
...
const handleSubmit = async () => {
  ...
  if (formData.value.delivery_date && isNaN(Date.parse(formData.value.delivery_date))) {
  ...
  try {
    ...
    if (data.success) {
      console.log('✅ Submission successful. Server returned:', data.data);
      showSuccess.value = true;

      // Optionally show confirmation alert
      alert(`Order ${data.data.order_no || props.order_no} updated successfully!`);
    } else {
    ...
    <div class="form-wrapper">
      ...
      <form @submit.prevent="handleSubmit" class="main-form">
        ...
        <div class="form-group">
          ...
          <input type="text" id="orderNo" v-model="formData.orderNo" readonly class="form-input input-readonly" />
        </div>
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <div class="input-icon-wrapper">
              ...
              <input type="tel" id="phone" v-model="formData.phone" placeholder="e.g., 0712345678" @blur="validatePhone"
                :class="['form-input', 'input-with-icon', { 'input-error': errors.phone }]" />
            </div>
          ...
          <div class="form-group">
            ...
            <div class="input-icon-wrapper">
              ...
              <input type="tel" id="altPhone" v-model="formData.altPhone" placeholder="e.g., 0723456789"
        ...
        <div class="form-group">
          ...
          <div class="input-icon-wrapper">
            ...
            <input type="text" id="address" v-model="formData.address" placeholder="Enter building, street, area"
              @input="showAddressSuggestions" @blur="hideSuggestions"
              :class="['form-input', 'input-with-icon', { 'input-error': errors.address }]" />
          </div>
          <span v-if="errors.address" class="error-message">{{ errors.address }}</span>

          <!-- Address Suggestions -->
          <div v-if="showSuggestions && addressSuggestions.length > 0" class="suggestions-dropdown">
            <div v-for="(suggestion, index) in addressSuggestions" :key="index" @mousedown="selectAddress(suggestion)"
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <select id="city" v-model="formData.city" @change="validateCity"
          ...
          <!-- Landmark -->
          <div class="form-group">
            <label for="landmark" class="form-label">Landmark/Building</label>
            <input type="text" id="landmark" v-model="formData.landmark" placeholder="e.g., Near Nakumatt"
              class="form-input" />
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input type="date" id="delivery_date" v-model="formData.delivery_date" :min="minDate" @change="validateDate"
            ...
            <span v-else class="helper-text">From tomorrow onwards</span>
          </div>

          <!-- Delivery Time -->
          <div class="form-group">
            <label for="deliveryTime" class="form-label">Preferred Time</label>
            <select id="deliveryTime" v-model="formData.deliveryTime" class="form-input">
        ...
        <div class="checkbox-container">
          <label class="checkbox-label">
            <input type="checkbox" v-model="formData.requestCallback" class="checkbox-input" />
            <span class="checkbox-content">
              <strong class="checkbox-title">Request a callback from our delivery team</strong>
              <small class="checkbox-subtitle">We'll call you to confirm the delivery details</small>
            </span>
          </label>
        </div>
        ...
        <div class="form-group">
          ...
          <textarea id="notes" v-model="formData.notes" rows="4" placeholder="Your message here..." maxlength="500"
            class="form-textarea"></textarea>
          <div class="char-counter">{{ formData.notes.length }}/500</div>
        </div>

        <button type="submit" class="submit-button" :disabled="isSubmitting"
          style="background-color: #2563eb; color: white; padding: 0.75rem 1.5rem; border-radius: 0.5rem; border: none; font-weight: 600;">
          <span v-if="!isSubmitting">Submit</span>
          <span v-else class="loading-state">
            <svg class="spinner" viewBox="0 0 24 24"
              style="width: 1rem; height: 1rem; margin-right: 0.5rem; vertical-align: middle;">
              <circle class="spinner-track" cx="12" cy="12" r="10" stroke="white" stroke-width="4" fill="none" />
              <path class="spinner-head" fill="white"
        ...
        <!-- Help Text -->
        <p class="help-text">
          Need help? Call us at <a href="tel:+254700000000" class="help-link">+254 700 000 000</a>
        </p>
      </form>
          ...
          <div v-if="showSuccess" class="modal-overlay" @click="closeModal">
            <div class="modal-content" @click.stop>
              <div class="success-icon-wrapper">
                <svg class="success-icon" width="60" height="60" viewBox="0 0 60 60" fill="none">
                  <circle cx="30" cy="30" r="28" stroke="#10b981" stroke-width="4" />
                  <path d="M20 30l8 8 12-16" stroke="#10b981" stroke-width="4" stroke-linecap="round"
                    stroke-linejoin="round" />
                </svg>
              </div>
              <h3 class="modal-title">Success!</h3>
              <p class="modal-text">We've received your updated delivery information. Our team will contact you soon.
              </p>
              <p class="modal-reference">Reference: {{ formData.orderNo }}</p>
