<template>
    <div>
        ...
        >
            <v-card class="modern-card">
                <v-card-title class="gradient-header">
                    <v-icon left color="white" size="24"
                        ...
                        <v-card-text>
                            ...
                            >
                                <template #item.waitTime="{ item }">
                                    <v-chip
                                        :color="getWaitTimeColor(item.waitTime)"
                                        size="small"
                                    >
...
const props = defineProps({
    modelValue: String,
    callData: Object,
    phoneNumber: String, // ✅ Already added from earlier
});
...
const filteredAgents = computed(() => {
    ...
    return agents.value.filter(
        (agent) =>
            agent.name
                .toLowerCase()
                .includes(agentSearch.value.toLowerCase()) ||
    ...
    try {
        ...
        if (transferTab.value === "number") {
            ...
            const agent = agents.value.find(
                (a) => a.id === transferForm.value.agent
            );
        ...
        emit("call-transferred", {
            target,
            warmTransfer: transferForm.value.warmTransfer,
        });
        closeDialog();
    } catch (error) {
...
// Add this watcher after your refs
watch(
    () => props.phoneNumber,
    (newPhone) => {
        if (newPhone && isNewCallDialog.value) {
            newCallForm.value.phone = newPhone;
        }
    },
    { immediate: true }
);
