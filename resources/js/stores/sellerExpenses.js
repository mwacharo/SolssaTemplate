...
export const useSellerExpensesStore = defineStore('sellerExpenses', () => {
  // State
  const expenses = ref([])
  ...
  const getExpensesBySeller = computed(() => {
    return (sellerId) => expenses.value.filter(expense => expense.seller_id === sellerId)
  })
  ...
  const createExpense = async (expenseData) => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        seller_id: parseInt(expenseData.seller_id),
        country_id: parseInt(expenseData.country_id),
        status: expenseData.status || 'not_applied',
        invoice_id: expenseData.invoice_id ? parseInt(expenseData.invoice_id) : null,
