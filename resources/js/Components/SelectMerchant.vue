    ...
    <div>
        ...
        >
            ...
            <template v-if="multiple" #selection="{ item, index }">
                ...
                <span v-if="index === maxChips" class="text-caption text-grey">
...
watch(
    () => props.modelValue,
    (val) => {
        internalValue.value = val ?? (props.multiple ? [] : null);
    },
);
...
const showError = computed(
    () => props.required && touched.value && !isValid.value,
);
...
const labelClass = computed(() =>
    props.required && !isValid.value && touched.value
        ? "text-red-600"
        : "text-gray-700",
);
